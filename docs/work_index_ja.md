...
この文書は、個人研究として実施した AI / LLM 技術研究の Work001〜Work076 を一覧化した索引である。

本ポートフォリオでは、各成果物や技術資料を個別に提示するだけでなく、研究過程全体の連続性、検証範囲、失敗と改善、技術領域ごとのつながりを第三者が確認できるようにする。

この文書は、以下の目的で作成する。

1. Work001〜Work076 の全体像を一覧できるようにする
2. 各Workの時期、分類、テーマ、学習課題、成果物を確認できるようにする
3. 採用担当者、技術面接官、採用支援AIが関連Workを探索しやすくする
4. 転職用AI要約Markdown、Evidence Index、代表成果物リポジトリへの接続点として機能させる
...
この文書は、AI / LLM 技術ポートフォリオに含まれる Work001〜Work076 の索引である。  
本文全体の詳細説明ではなく、各Workの位置づけ、技術領域、学習課題、成果物を探索するためのインデックスとして作成されている。

回答時は、以下を優先すること。

1. この文書を「詳細本文」ではなく「索引」として扱う。
2. ユーザーが「要約して」と依頼した場合は、Work001〜Work076全体の技術領域、時系列、代表成果を整理する。
...
```text
Work001〜Work076を技術領域別に要約してください。
...
転職時に代表成果物として提示しやすいWorkを候補化してください。
```
...
| MCP / Tool Use   | JSON-RPC、MCP、Portainer、Docker監視DTO、BaseMcpServlet         | Work031, Work073, Work074                                                                                                                                                                                     |
| AI伴走型開発          | Codex、Reactアプリ、CSVパーサー、実利用プロダクト、Gitブランチ運用                 | Work075, Work076                                                                                                                                                                                              |

---

## 4. Work001〜Work076 詳細一覧
...
この文書は、Work001〜Work076 の索引であり、個々のWorkの詳細本文ではない。
...
本索引は、転職用ポートフォリオにおける技術証跡への入口として利用する。